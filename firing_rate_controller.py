...
class FiringRateController:
    ...
    def __init__(self, pars):
        ...
        self.all_muscles = 4*self.n_neurons + np.arange(0, 2*self.n_muscle_cells)
        # all muscle cells indexes
    ...
    def motor_output(self, iteration):
        ...
        self.state[iteration, self.muscle_l] = left_muscles
        self.state[iteration, self.muscle_r] = right_muscles
    ...
    def ode_rhs(self, _time, state, pos=None):
        ...
        for i in range(self.n_neurons):
            W_in[i, i:min(i + self.pars.n_asc + 1, self.n_neurons)] = 1 / np.arange(1, min(self.pars.n_asc + 1, self.n_neurons - i) + 1)
            W_in[i, max(i - self.pars.n_desc, 0):i] = 1 / np.arange(1, i - max(i - self.pars.n_desc, 0) + 1)[::-1]
        ...
        self.dstate[self.left_v] = (-state[self.left_v] + np.sqrt(np.maximum(
            input_left - self.pars.w_inh * (W_in @ state[self.right_v]), 0))) / self.pars.tau
        self.dstate[self.right_v] = (-state[self.right_v] + np.sqrt(np.maximum(
            input_right - self.pars.w_inh * (W_in @ state[self.left_v]), 0))) / self.pars.tau
        
        # Muscle dynamics
        self.dstate[self.left_m] = (self.pars.w_V2a2muscle * (W_mc @ state[self.left_v]) * (1 - state[self.left_m]) / self.pars.taum_a
                                    - state[self.left_m] / self.pars.taum_d)
        self.dstate[self.right_m] = (self.pars.w_V2a2muscle * (W_mc @ state[self.right_v]) * (1 - state[self.right_m]) / self.pars.taum_a
